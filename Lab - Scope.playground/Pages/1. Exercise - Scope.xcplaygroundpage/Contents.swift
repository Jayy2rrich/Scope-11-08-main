...
//print("The value of foo is \(foo)")
//It will generate a complier error because the scope is local and not global scope
...
//The scope x is a global scope and when it gets into the flor loop it gets modtified inside the for loop. The only scope that is modtified is the one inside of the loop. foo is described in the local for loop.
